...
# Butterfly mode uses BUTTERFLY_ASCII from security_wizard.jl directly
...
const BUTTERFLY_PREVIEW_LINES = split(BUTTERFLY_ASCII, '\n')[1:min(10, end)]

# b2 tachikoma bot (from art.jl) for neuromancer preview
const NEURO_BOT_ASCII = raw"""
       _____                  ______
    __/ / \ \                / /  \ \__
  _/ ---   \ \              / /    --- \_
 / /  ****  \  | O-     -O | /   **** \  \
...
function setup_intro_animations!(m::SetupWizardModel)
    ...
    elseif m.mode == L33T
        # Initialize rain columns — full width
        ...
        # Bot materialization: slower, starts at frame 90
        animate!(
            m.animator,
            :neuro_bot,
            tween(0.0, 1.0; duration = 120, easing = ease_out_cubic),
        ...
        animate!(
            m.animator,
            :neuro_scan,
            tween(0.0, 1.0; duration = 60, easing = linear, loop = :loop),
        )
        m.typed_text = ""
        m.typed_index = 0
    end
...
function update_neuromancer_anim!(m::SetupWizardModel)
    # Matrix rain: variable speed per column (some fall faster)
    for i in eachindex(m.rain_columns)
        speed = (i % 3 == 0) ? 2 : 1  # every 3rd column falls faster
        m.rain_columns[i] += speed
        if m.rain_columns[i] > 45
            m.rain_columns[i] = 1
            m.rain_chars[i] = rand([
                '0',
                '1',
    ...
    # Typewriter effect: 1 char per 3 frames, starting at frame 150
    if m.tick >= 150 && m.typed_index < length(m.typed_target)
        if (m.tick - 150) % 3 == 0
            m.typed_index += 1
...
function view_mode_select(m::SetupWizardModel, f::Frame)
    ...
    mode_previews = [DRAGON_PREVIEW_LINES, BUTTERFLY_PREVIEW_LINES, NEURO_BOT_LINES]
...
function view_neuromancer_intro(m::SetupWizardModel, area::Rect, buf::Buffer)
    # Matrix rain — longer trails, variable speed per column
    for (col_idx, col_y) in enumerate(m.rain_columns)
        col_idx > area.width && break
        x = area.x + col_idx - 1
        trail_len = 4 + (col_idx % 5)  # variable trail length per column
        for row_off = 0:min(trail_len, area.height - 1)
            y = area.y + (col_y + row_off - 1) % area.height
            if y >= area.y && y <= bottom(area) && x >= area.x && x < right(area)
                brightness = max(1, length(NEURO_GREENS) - row_off)
                ch = if row_off == 0
                    m.rain_chars[col_idx]
                elseif row_off == 1
                    rand(['0', '1', 'ﾊ', 'ﾐ', 'ﾋ', 'ｰ'])
                elseif row_off <= 3
                    rand(['0', '1', ':', '.'])
                ...
                set_char!(
                    ...
                    y,
                    ch,
                    Style(; fg = Color256(NEURO_GREENS[brightness]), dim = (row_off > 2)),
                )
            end
        end
    end

    # Scanline sweep — horizontal bright line moving down the screen
    scan = val(m.animator, :neuro_scan)
    scan_y = area.y + Int(round(scan * area.height))
    if scan_y >= area.y && scan_y <= bottom(area)
        for x = area.x:min(area.x + area.width - 1, right(area))
            set_char!(
                buf,
                x,
                scan_y,
                '─',
                Style(; fg = Color256(NEURO_GREENS[end]), dim = true),
            )
        end
    end

    # Bot materialization (starts at frame 90, slower reveal)
    if m.tick >= 90
        bot_lines = NEURO_BOT_LINES
        bot_reveal = val(m.animator, :neuro_bot)
        bot_width = maximum(length.(bot_lines); init = 0)
        bot_x = max(area.x, area.x + (area.width - bot_width) ÷ 2)
        bot_y = max(area.y + 2, area.y + (area.height - length(bot_lines)) ÷ 2 - 2)

        for (i, line) in enumerate(bot_lines)
            # Staggered per-line reveal
            line_frac = (i - 1) / max(1, length(bot_lines) - 1)
            line_reveal = clamp((bot_reveal - line_frac * 0.4) / 0.6, 0.0, 1.0)
            line_reveal <= 0.0 && continue

            y = bot_y + i - 1
            y > bottom(area) - 5 && break

            for (j, ch) in enumerate(line)
                x = bot_x + j - 1
                x >= right(area) - 1 && break
                x < area.x && continue
                # Glitch: probability decreases as line solidifies
                glitch_prob = (1.0 - line_reveal) * 0.8
                display_ch = if rand() < glitch_prob
                    rand(['0', '1', '#', '@', '%', '$', '&'])
                else
                    ch
                end
                # Bright green for solid chars, dimmer for glitchy
                cidx =
                    line_reveal > 0.9 ? mod1(i + j, length(NEURO_GREENS)) :
                    mod1(m.tick ÷ 2 + i + j, length(NEURO_GREENS))
                style =
                    Style(; fg = Color256(NEURO_GREENS[cidx]), bold = (line_reveal > 0.8))
                set_char!(buf, x, y, display_ch, style)
            end
        end
    end

    # Typewriter text — two lines for more presence
...
function render_companion_art(m::SetupWizardModel, area::Rect, buf::Buffer)
    lines = if m.mode == STANDARD
        COMPANION_WIZ
    elseif m.mode == GENTLE
        COMPANION_WIZ_B
    else
        COMPANION_B2[1:min(10, length(COMPANION_B2))]
    end
    ...
    # Gentle mode: draw small butterflies around the wizard
    if m.mode == GENTLE
        butterfly_positions = [
    ...
    # Sparkles around the wizard (standard + gentle modes)
    if m.mode == STANDARD || m.mode == GENTLE
        sparkle_chars = ['✧', '⋆', '*', '.', ':', '+']
        sparkle_colors = m.mode == GENTLE ? BUTTERFLY_COLORS : WIZARD_COLORS
        for si = 1:8
            ...
            if visible
                ...
                cidx = mod1(m.tick ÷ 6 + si, length(sparkle_colors))
                set_char!(
                    buf,
                    sx,
                    sy,
                    ch,
                    Style(; fg = Color256(sparkle_colors[cidx]), dim = true),
                )
            end
        end
    end
...
function render_dim_rain(m::SetupWizardModel, area::Rect, buf::Buffer)
    for col_idx = 1:min(length(m.rain_columns), area.width)
        x = area.x + col_idx - 1
        ...
        if y >= area.y && y <= bottom(area) && x >= area.x && x < right(area)
            set_char!(
                ...
                y,
                m.rain_chars[col_idx],
                Style(; fg = Color256(NEURO_GREENS[1]), dim = true),
            )
        end
    end
end
